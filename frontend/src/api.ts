...
function buildUrl(path:string){const p=path.startsWith("/")?path:`/${path}`;if(p.startsWith("/auth/")||p.startsWith("/public/")||p==="/health"||p==="/dashboard")return `${API_URL}${p}`;return `${API_URL}/api${p}`;}

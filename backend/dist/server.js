...
const param = (req, name) => { const value = req.params[name]; if (!value?.trim())
    throw new Error(`Parámetro inválido: ${name}`); return value; };
function auth(req, res, next) { const value = req.headers.authorization; if (!value?.startsWith("Bearer ")) {
    res.status(401).json({ message: "Debes iniciar sesión." });
    return;
} try {
    req.auth = jwt.verify(value.slice(7), jwtSecret);
    next();
}
catch {
    res.status(401).json({ message: "La sesión ha caducado." });
} }
async function ensureAdmin() { const email = (process.env.ADMIN_EMAIL || "admin@rachelstudio.es").trim().toLowerCase(); const password = process.env.ADMIN_PASSWORD || "Rachel1234"; if (!await prisma.user.findUnique({ where: { email } }))
    await prisma.user.create({ data: { name: "Rachel Studio", email, passwordHash: await bcrypt.hash(password, 12) } }); }
function mailer() { if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASS)
    return null; return nodemailer.createTransport({ host: process.env.SMTP_HOST, port: Number(process.env.SMTP_PORT || 587), secure: process.env.SMTP_SECURE === "true", auth: { user: process.env.SMTP_USER, pass: process.env.SMTP_PASS } }); }
async function sendMail(subject, html, to = process.env.NOTIFICATION_EMAIL || process.env.ADMIN_EMAIL) { const transport = mailer(); if (!transport || !to) {
    console.log("Correo no enviado: configura SMTP_HOST, SMTP_USER, SMTP_PASS y NOTIFICATION_EMAIL.");
    return;
} await transport.sendMail({ from: process.env.SMTP_FROM || process.env.SMTP_USER, to, subject, html }); }
const fullAppointment = { client: true, employee: true, category: true, services: true, products: { include: { product: true } } };
const fullRequest = { client: true, employee: true, category: true, services: true };
...
async function appointmentData(data) { const services = await prisma.service.findMany({ where: { id: { in: data.services.map(x => x.serviceId) }, active: true }, include: { category: true } }); if (services.length !== data.services.length)
    throw new Error("Alguno de los servicios no existe o está desactivado."); const lines = data.services.map(item => { const service = services.find(s => s.id === item.serviceId); const finalPrice = Number(service.price) * (1 - item.discount / 100); return { serviceId: service.id, serviceName: service.name, durationMin: service.durationMin, unitPrice: service.price, discount: new Prisma.Decimal(item.discount), finalPrice: new Prisma.Decimal(finalPrice) }; }); const startsAt = new Date(data.startsAt); const endsAt = new Date(startsAt.getTime() + lines.reduce((a, b) => a + b.durationMin, 0) * 60000); return { lines, startsAt, endsAt, total: new Prisma.Decimal(lines.reduce((a, b) => a + Number(b.finalPrice), 0)), categoryId: services[0]?.categoryId || null }; }
async function isAvailable(startsAt, endsAt, employeeId, ignoreAppointment) { const employeeWhere = employeeId ? { employeeId } : {}; const overlap = { startsAt: { lt: endsAt }, endsAt: { gt: startsAt } }; const [appointment, absence] = await Promise.all([prisma.appointment.findFirst({ where: { ...employeeWhere, ...overlap, id: ignoreAppointment ? { not: ignoreAppointment } : undefined, status: { not: AppointmentStatus.CANCELADA } } }), prisma.absence.findFirst({ where: { ...(employeeId ? { employeeId } : {}), ...overlap } })]); return !appointment && !absence; }
app.get("/", (_req, res) => res.json({ name: "Rachel Studio API", status: "ok" }));
app.get("/health", (_req, res) => res.json({ status: "ok" }));
app.post("/auth/login", asyncRoute(async (req, res) => { const data = z.object({ email: z.string().email(), password: z.string().min(6) }).parse(req.body); const user = await prisma.user.findUnique({ where: { email: data.email.toLowerCase() } }); if (!user || !await bcrypt.compare(data.password, user.passwordHash)) {
    res.status(401).json({ message: "Correo o contraseña incorrectos." });
    return;
} res.json({ token: jwt.sign({ userId: user.id, email: user.email }, jwtSecret, { expiresIn: "7d" }), user: { id: user.id, name: user.name, email: user.email } }); }));
app.get("/auth/me", auth, asyncRoute(async (req, res) => { const user = await prisma.user.findUnique({ where: { id: req.auth.userId }, select: { id: true, name: true, email: true } }); if (!user) {
...
for (const [path, model, schema] of [["employees", "employee", employeeSchema], ["categories", "category", categorySchema]]) {
    app.get(`/api/${path}`, auth, asyncRoute(async (_req, res) => { res.json(await prisma[model].findMany({ orderBy: { name: "asc" } })); }));
    app.post(`/api/${path}`, auth, asyncRoute(async (req, res) => { res.status(201).json(await prisma[model].create({ data: schema.parse(req.body) })); }));
    app.put(`/api/${path}/:id`, auth, asyncRoute(async (req, res) => { res.json(await prisma[model].update({ where: { id: param(req, "id") }, data: schema.parse(req.body) })); }));
}
...
app.post("/api/products", auth, asyncRoute(async (req, res) => { const d = productSchema.parse(req.body); const product = await prisma.product.create({ data: { ...d, quantity: new Prisma.Decimal(d.quantity), minimum: new Prisma.Decimal(d.minimum), cost: new Prisma.Decimal(d.cost), price: new Prisma.Decimal(d.price) } }); if (d.quantity > 0)
    await prisma.stockMovement.create({ data: { productId: product.id, type: StockMovementType.ENTRADA, quantity: new Prisma.Decimal(d.quantity), previousStock: new Prisma.Decimal(0), resultingStock: new Prisma.Decimal(d.quantity), reason: "Stock inicial" } }); res.status(201).json(product); }));
...
app.post("/api/appointments", auth, asyncRoute(async (req, res) => { const d = appointmentSchema.parse(req.body); const calc = await appointmentData(d); if (!await isAvailable(calc.startsAt, calc.endsAt, d.employeeId))
    throw new Error("La empleada no está disponible en ese horario."); res.status(201).json(await prisma.appointment.create({ data: { clientId: d.clientId, employeeId: d.employeeId, categoryId: calc.categoryId, startsAt: calc.startsAt, endsAt: calc.endsAt, status: d.status, notes: d.notes, paid: d.paid, total: calc.total, services: { create: calc.lines }, products: { create: d.products.map(p => ({ productId: p.productId, quantity: new Prisma.Decimal(p.quantity) })) } }, include: fullAppointment })); }));
app.put("/api/appointments/:id", auth, asyncRoute(async (req, res) => { const id = param(req, "id"), d = appointmentSchema.parse(req.body), calc = await appointmentData(d); if (!await isAvailable(calc.startsAt, calc.endsAt, d.employeeId, id))
...
app.post("/api/absences", auth, asyncRoute(async (req, res) => { const d = absenceSchema.parse(req.body); if (new Date(d.endsAt) <= new Date(d.startsAt))
    throw new Error("La fecha final debe ser posterior a la inicial."); res.status(201).json(await prisma.absence.create({ data: { ...d, startsAt: new Date(d.startsAt), endsAt: new Date(d.endsAt) }, include: { employee: true } })); }));
...
app.post("/api/booking-requests/:id/decision", auth, asyncRoute(async (req, res) => { const id = param(req, "id"); const d = z.object({ action: z.enum(["accept", "deny", "propose"]), proposedAt: z.string().datetime().optional(), employeeId: z.string().optional(), notes: z.string().optional() }).parse(req.body); const booking = await prisma.bookingRequest.findUniqueOrThrow({ where: { id }, include: fullRequest }); if (d.action === "deny") {
    const updated = await prisma.bookingRequest.update({ where: { id }, data: { status: BookingRequestStatus.DENEGADA, adminNotes: d.notes || null }, include: fullRequest });
    await sendMail("Solicitud de cita no disponible", `<p>Hola ${booking.client.alias || booking.client.name}, no podemos confirmar el horario solicitado.</p><p>${d.notes || "Puedes enviar una nueva solicitud desde la web."}</p>`, booking.client.email || undefined);
    res.json(updated);
    return;
} if (d.action === "propose") {
    if (!d.proposedAt)
        throw new Error("Indica la nueva fecha y hora.");
    const updated = await prisma.bookingRequest.update({ where: { id }, data: { status: BookingRequestStatus.PROPUESTA_ENVIADA, proposedAt: new Date(d.proposedAt), employeeId: d.employeeId || booking.employeeId, adminNotes: d.notes || null }, include: fullRequest });
    await sendMail("Te proponemos otra hora", `<p>Hola ${booking.client.alias || booking.client.name}, te proponemos <b>${new Date(d.proposedAt).toLocaleString("es-ES")}</b>.</p><p>${d.notes || "Responde a este correo para confirmarla."}</p>`, booking.client.email || undefined);
    res.json(updated);
    return;
} const employeeId = d.employeeId || booking.employeeId; if (!employeeId)
    throw new Error("Selecciona una empleada."); const start = booking.proposedAt || booking.requestedAt; const end = new Date(start.getTime() + booking.services.reduce((s, x) => s + x.durationMin, 0) * 60000); if (!await isAvailable(start, end, employeeId))
...
app.use((error, _req, res, _next) => { console.error(error); if (error instanceof z.ZodError) {
    const fields = Object.fromEntries(error.issues.map(i => [String(i.path[0] || "form"), i.message]));
    res.status(400).json({ message: "Revisa los campos marcados.", fields });
    return;

...
import { AppointmentStatus, Prisma, PrismaClient, ProductUnit, StockMovementType } from "@prisma/client";
import { z } from "zod";
...
app.post("/api/products", auth, asyncRoute(async (req, res) => {
    ...
    if (data.quantity > 0)
        await prisma.stockMovement.create({ data: { productId: product.id, type: StockMovementType.ENTRADA, quantity: new Prisma.Decimal(data.quantity), previousStock: new Prisma.Decimal(0), resultingStock: new Prisma.Decimal(data.quantity), reason: "Stock inicial" } });
...
app.put("/api/products/:id", auth, asyncRoute(async (req, res) => {
    ...
    const next = new Prisma.Decimal(data.quantity);
    const diff = next.minus(current.quantity);
    const product = await prisma.$transaction(async (tx) => {
        ...
        if (!diff.isZero())
            await tx.stockMovement.create({ data: { productId: id, type: StockMovementType.AJUSTE, quantity: diff, previousStock: current.quantity, resultingStock: next, reason: "Edición manual" } });
        return updated;
    });
    res.json(product);
}));
app.post("/api/products/:id/movements", auth, asyncRoute(async (req, res) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        const isOutgoing = data.type === StockMovementType.SALIDA ||
        ...
        const signed = isOutgoing
            ...
            : data.type === StockMovementType.ENTRADA
                ? raw.abs()
                : raw;
        const next = product.quantity.plus(signed);
        if (next.lessThan(0))
...
app.post("/api/appointments", auth, asyncRoute(async (req, res) => {
    ...
    if (catalog.length !== data.services.length)
        throw new Error("Alguno de los servicios no existe o está desactivado.");
    ...
    const totalMinutes = lines.reduce((sum, line) => sum + line.durationMin, 0);
    const startsAt = new Date(data.startsAt);
    const endsAt = new Date(startsAt.getTime() + totalMinutes * 60000);
    const total = lines.reduce((sum, line) => sum + Number(line.finalPrice), 0);
...
app.put("/api/appointments/:id", auth, asyncRoute(async (req, res) => {
    const id = param(req, "id");
    const data = appointmentSchema.parse(req.body);
    const catalog = await prisma.service.findMany({ where: { id: { in: data.services.map(service => service.serviceId) } } });
    const lines = data.services.map(item => { const service = catalog.find(value => value.id === item.serviceId); if (!service)
        throw new Error("Servicio no encontrado."); const finalPrice = Number(service.price) * (1 - item.discount / 100); return { serviceId: service.id, serviceName: service.name, durationMin: service.durationMin, unitPrice: service.price, discount: new Prisma.Decimal(item.discount), finalPrice: new Prisma.Decimal(finalPrice) }; });
    const startsAt = new Date(data.startsAt);
    const endsAt = new Date(startsAt.getTime() + lines.reduce((sum, line) => sum + line.durationMin, 0) * 60000);
    const total = lines.reduce((sum, line) => sum + Number(line.finalPrice), 0);
...
app.post("/api/appointments/:id/finish", auth, asyncRoute(async (req, res) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (appointment.status === AppointmentStatus.FINALIZADA)
            return appointment;
        for (const usage of appointment.products) {
            const product = await tx.product.findUniqueOrThrow({ where: { id: usage.productId } });
            const next = product.quantity.minus(usage.quantity);
            if (next.lessThan(0))
...
app.use((error, _req, res, _next) => {
    ...
    if (error instanceof z.ZodError) {
        res.status(400).json({ message: "Revisa los datos introducidos.", errors: error.issues });
        return;
    }
